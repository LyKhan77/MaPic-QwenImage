...
*   **Generation Queue:** Submit another prompt while an image is generating; MaPic queues it and runs it after the active generation completes.
...
5.  While an image is generating, use **New Generation** or the bottom prompt input to submit more prompts into the queue.
6.  View your creation in the main canvas.
7.  Use the **Sidebar** to access previous generations or switch themes.

MaPic accepts a maximum of 10 active or queued generations globally across all users. When the queue is full, new requests are rejected with a clear retry-later message.
