...
- **Remove Background (CPU, 2026-09-25):** tombol gunting tunggal di prompt bar (di samping ikon Settings) — mati = Generate seperti biasa, hidup = potong latar **satu** gambar tanpa prompt. Jalur inferensi **terpisah sepenuhnya** dari Qwen/GPU: library `rembg` + ONNX `isnet-general-use` berjalan di CPU di container `backend`, jadi model Qwen `unloaded` maupun GPU sibuk tidak menghalangi. Terukur di container server: panggilan pertama 0,89 s lalu 0,39 s untuk 512², RSS ~2,0 GiB, RGB subjek diambil dari gambar sumber (bukan dari model), dan keluaran identik antar pemanggilan. Bobot tinggal di host `~/apps/mapic-rembg` dan di-mount read-only (`REMBG_HOME=/models`); tanpa bobot endpoint balas `503` dan **tidak** mengunduh saat request. Batas mutu: objek buram berlatar bersih bagus, objek bening (kaca/es) belum layak.
- **Lisensi bobot cutout — INTERNAL USE ONLY (keputusan 2026-09-25):** bobot `isnet-general-use.onnx` **tidak memiliki pernyataan lisensi**. Repo DIS ber-Apache-2.0 hanya untuk *"code and evaluation metric"* (bukan bobot), dan dataset latih DIS5K non-komersial tanpa izin. Issue DIS [#150](https://github.com/xuebinqin/DIS/issues/150) (permintaan izin komersial, dibuka 2026-08-20) masih **terbuka tanpa jawaban maintainer**. Pengguna memutuskan: **alat internal saja**. Karena itu: (a) bobot **tidak** boleh masuk git atau image — saat ini hanya ada di host dan ter-mount read-only, dan `git ls-files` mengkonfirmasi nol berkas bobot ter-track; (b) jangan dipakai untuk pekerjaan berbayar pihak luar; (c) jangan didistribusikan ke server/klien lain. Sebelum salah satu dari itu terjadi, selesaikan izin tertulis **atau** ganti ke model berlisensi jelas (kandidat: BiRefNet, repo MIT — bobotnya tetap perlu diverifikasi dan di-benchmark ulang). Catatan ini bukan nasihat hukum.
