...
- **Generation queue UX:** Loading screen is read-only and shows no prompt input; users click New Generation during active work to open one clean prompt input and submit additional backend-queued jobs. Completed current-user jobs automatically open their result canvas, including after refresh-time active job recovery.
