...
**Susulan (commit `fix: stop chained label when reusing a cutout as source`).** Uji browser nyata menemukan label yang menumpuk begitu sumber cutout diambil dari riwayat: prompt item itu sendiri sudah berbentuk label cutout, sedangkan `Dashboard.handleRemoveBackground` memakai `currentGen?.prompt` apa adanya, sehingga tersimpan `Remove background — Remove background — cocacola-can.png`. Perbaikan di sisi klien lewat helper murni baru `cutoutLabelFromPrompt(prompt?)` di `frontend/src/lib/removeBackground.ts`: label telanjang persis `Remove background` (baris lama) menjadi `undefined` sehingga server menyimpan label telanjang, bentuk berprefiks `Remove background — ` dikupas lalu di-trim (`Remove background — cocacola-can.png` → `cocacola-can.png`), sedangkan teks lain diteruskan utuh — termasuk `Remove background of my photo` dan nama berkas unggahan seperti `product.png`, yang **tidak** pernah lewat helper ini karena jalur unggahan tetap mengirim `file.name`. Literal em dash U+2014 kini tinggal di satu konstanta `CUTOUT_LABEL_SEPARATOR` yang dipakai `isCutoutGeneration` maupun pengupasan prefiks. Bukti ulang: `npm --prefix frontend run check:remove-background` → `cutoutLabelFromPrompt de-chains cutout sources OK` (kasus label bersufiks, label telanjang, prompt biasa, prompt mirip-prefiks, `null`/`undefined`/kosong, dan `product.png` utuh); `npm --prefix frontend run build` → `✓ built in 1.79s`; `npm --prefix frontend run lint` → `✖ 5 problems (5 errors, 0 warnings)` seperti sebelumnya; `SUPABASE_URL=https://example.supabase.co SUPABASE_SERVICE_ROLE_KEY=test backend/.venv/bin/python -m unittest discover -s backend/tests` → `Ran 42 tests in 0.209s` `OK`. Tanpa perubahan backend maupun DB.
